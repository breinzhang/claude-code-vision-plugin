...
export function listVisionTools() {
    return [
        {
            name: 'analyze_image',
...
export function handleAnalyzeImageResult(result) {
    return {
        content: [{ type: 'text', text: result.markdown }],
        structuredContent: result,
    };
}
export function sanitizeDoctorOutput(value) {
    if (Array.isArray(value))
        return value.map((item) => sanitizeDoctorOutput(item));
...
export async function handleMcpToolCall(call) {
...
export async function createMcpServer() {
    const server = new Server({ name: 'vision-bridge', version: '0.1.1' }, { capabilities: { tools: {} } });
